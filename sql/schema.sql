...
-- ---------- lock the tables down from Supabase's public API defaults ----------
-- The console never talks to Supabase from the browser, only from
-- server-side route handlers using the service role key, which
-- bypasses Row Level Security entirely. Enabling RLS with no policies
-- means that even if these tables were ever exposed to the anon/public
-- API, nothing could be read or written without the service role key.
