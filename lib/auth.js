 ...
 * - Re-checks the session's version against the CURRENT version stored
 *   in the database, so a password change immediately invalidates every
 *   other open session (including one on a stolen device).
 */
async function isAuthenticated(req) {
  ...
  try {
    ...
    return false; // fail closed if we can't reach the database
  }

  return true;
}

/** Express middleware for HTML page routes. Redirects to /login.html. */
