...
if (process.env.AUTH_TEST_MODE === '1') {
  ...
  module.exports = {
    ...
    async getDoc(section) {
      return docs[section] || { section, content: '', updated_at: null };
    },
    async saveDoc(section, content) {
      const doc = { section, content, updated_at: new Date().toISOString() };
      docs[section] = doc;
      return doc;
    },
    async listDocs(sections) {
...
function createSupabaseBackedModule() {
  ...
  async function getDoc(section) {
    ...
    return data || { section, content: '', updated_at: null };
  }
  ...
  async function listDocs(sections) {
    const supabase = getAdminClient();
    const { data, error } = await supabase
      .from(DOCS_TABLE)
    ...
    return sections.map((section) => bySection.get(section) || { section, updated_at: null });
  }
