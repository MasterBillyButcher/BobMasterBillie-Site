...
if (process.env.AUTH_TEST_MODE === '1') {
  ...
  module.exports = {
    ...
    async getPrefs() {
      return { pinned_sections: [...prefs.pinned_sections] };
    },
    async savePrefs(pinnedSections) {
      prefs.pinned_sections = [...pinnedSections];
      return { pinned_sections: [...prefs.pinned_sections] };
    },
...
function createSupabaseBackedModule() {
  ...
  /**
   * User preferences (currently just pinned sidebar sections). One row
   * in dashboard_prefs, same lockdown pattern as everything else here:
   * RLS on, no anon/authenticated policies, service-role only.
   */
  const PREFS_TABLE = 'dashboard_prefs';
  ...
  async function getPrefs() {
    ...
    const { data, error } = await supabase
      .from(PREFS_TABLE)
      .select('pinned_sections')
      .eq('id', PREFS_ROW_ID)
      .maybeSingle();

    if (error) throw new Error('Failed to load preferences.');
    return { pinned_sections: (data && data.pinned_sections) || [] };
  }

  async function savePrefs(pinnedSections) {
    const supabase = getAdminClient();
    const { data, error } = await supabase
      .from(PREFS_TABLE)
      .upsert(
    ...
    return { pinned_sections: data.pinned_sections || [] };
  }
