...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /**
   * Strips decorative leading characters (emoji, pictographs, variation
   * selectors, stray punctuation) so copying a row yields the actual
   * command starting at its prefix — "🛡️ /mods" copies as "/mods",
   * "🎵 !sr [url]" as "!sr [url]". If a line has no recognizable
   * command prefix, the trimmed text is returned unchanged.
   */
  function stripLeadingDecoration(text) {
    const prefixMatch = text.match(/[!\/$@#][^\s]*/);
    if (prefixMatch) {
      return text.slice(prefixMatch.index).trim();
    }
    // No command prefix — drop any leading emoji/symbol run anyway.
    return text
      .replace(
        /^(?:[\p{Extended_Pictographic}\p{Emoji_Presentation}\uFE0F\u200D\u20E3\s]|[\p{S}])+/u,
        ''
      )
      .trim();
  }
