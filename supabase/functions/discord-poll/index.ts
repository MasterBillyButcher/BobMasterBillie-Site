...
interface DiscordAuditLogChange {
  key: string;
  new_value?: unknown;
}
...
async function fetchNewModActions(guildId: string, after: string | null): Promise<ModActionEntry[]> {
  ...
  for (const actionType of ACTION_TYPES) {
    ...
    const usersById: Record<string, DiscordUser> = Object.fromEntries(
      (data.users || []).map((u: DiscordUser) => [u.id, u])
    );

    for (const entry of (data.audit_log_entries || []) as DiscordAuditLogEntry[]) {
...
function normaliseEntry(entry: DiscordAuditLogEntry, actionType: number, usersById: Record<string, DiscordUser>): ModActionEntry {
  const target = entry.target_id ? usersById[entry.target_id] : undefined;
