...
// Auth API
// ---------------------------------------------------------------------------
// Rate limiting is a production concern — skip it under AUTH_TEST_MODE so
// a single long-lived test server (many logins across many test cases)
// doesn't trip its own limits and produce flaky, order-dependent failures.
const skipInTestMode = () => process.env.AUTH_TEST_MODE === '1';
...
// ---------------------------------------------------------------------------
// Prefs API — user preferences (currently just pinned sidebar sections).
// Stored server-side in Supabase so they follow you across browsers and
// devices, instead of being stuck in one browser's localStorage.
// ---------------------------------------------------------------------------
app.get('/api/prefs', requireAuthApi, async (req, res) => {
  try {
...
app.put('/api/prefs', requireAuthApi, verifyCsrf, async (req, res) => {
  const { pinned_sections: pinnedSections } = req.body || {};

  if (!Array.isArray(pinnedSections) || !pinnedSections.every((s) => typeof s === 'string')) {
    return res.status(400).json({ error: 'pinned_sections must be an array of strings.' });
  }
  const unknown = pinnedSections.filter((s) => !VALID_SECTIONS.has(s));
  if (unknown.length > 0) {
    return res.status(400).json({ error: `Unknown section(s): ${unknown.join(', ')}` });
  }

  try {
    const prefs = await savePrefs([...new Set(pinnedSections)]);
    res.json({ prefs });
  } catch (err) {
